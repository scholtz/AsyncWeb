...
class Client {
    public static $curlOptions = array(CURLOPT_HEADER => false, CURLOPT_RETURNTRANSFER => true,);
    public static function Call($url, $variables = array(), $type = null) {
        ...
        if ($info["http_code"] != "200") {
            throw new \Exception(\AsyncWeb\System\Language::get("API Server returned http status code %code% %error%", array("%error%" => curl_error($curl), "%code%" => $info["http_code"])));
        }
        if (!$response) {
