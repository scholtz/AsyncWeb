    ...
    public static function ConvertQuery($QueryBuilder = array(), $CONVERT = array(), $DB_DICT_COLS = array()) {
        ...
        if (isset($QueryBuilder["Where"])) {
            ...
            foreach ($QueryBuilder["Where"] as $k => $v) {
                if (is_numeric($k) && !is_array($v) && (trim($v) == "-(" || trim($v) == "-)" || strtolower(trim($v)) == "-or" || strtolower(trim($v)) == "-and")) {
                    // valid values
                    $where[] = array("col" => strtolower(trim($v)));
                } elseif (is_numeric($k) && is_array($v) && count($v) == 1 && isset($v[0]) && (trim($v[0]) == "-(" || trim($v[0]) == "-)" || strtolower(trim($v[0])) == "-or" || strtolower(trim($v[0])) == "-and")) {
                    // valid values
