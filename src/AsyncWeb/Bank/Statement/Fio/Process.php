...
class Processor implements \AsyncWeb\Bank\Statement\ProcessorInterface{
	...
	public function ProcessStatement(){
		$token = $this->token;
		$new = array();
		if($this->debug){echo "downloading statement\n";}
		$xmltext = $this->getVypisXML($token);
		
		if($this->debug) var_dump(file_put_contents("test03.html",$xmltext));

		if(strlen($xmltext) < 10){echo "E00\n";\AsyncWeb\Cron\CLI::e();}
		...
		if(!$xpath->query("//accountId")->item(0)) {echo "E03\n";\AsyncWeb\Cron\CLI::e();}
		...
		foreach($xpath->query("//Transaction") as $transaction){
			...
			$col = "column_1"; $dbcol = "value"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "currency";$col ="column_14"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "date";$col ="column_0"; if($xpath->query($col,$transaction)->item(0)) $date = $data[$dbcol] = date("Y-m-d",$time = strtotime($xpath->query($col,$transaction)->item(0)->nodeValue));
			if($time < strtotime("2013-07-24")) continue;
			$dbcol = "bank";$col ="column_3"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "account";$col ="column_2"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "note";$col ="column_16"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "var";$col ="column_5"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "const";$col ="column_4"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			$dbcol = "spec";$col ="column_6"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
			...
			$dbcol = "id2";$col ="column_22"; if($xpath->query($col,$transaction)->item(0)) $data[$dbcol] = $xpath->query($col,$transaction)->item(0)->nodeValue;
	...
	private function getXMLStatement($token){
		...
		$data = \AsyncWeb\Connectors\MyCurl::getPage($page,array(),array());//CURLOPT_SSL_VERIFYPEER=>false
