...
class Cache {
    ...
    public static function set($key, $group, $value) {
        ...
        if (Cache::$debug) {
            ...
            $line = microtime(true) . ":E:";
            foreach ($debug as $dbg) {
                if (isset($dbg["function"])) $line.= "F=" . $dbg["function"] . ":A=";
                foreach ($dbg["args"] as $k => $v) $line.= "$k=$v;";
            }
            $line.= "\n";
    ...
    private static function delTree($dir) {
        ...
        foreach ($files as $file) {
            if (substr($file, -1) == '/') Cache::delTree($file);
            else unlink($file);
        }
        rmdir($dir);
    }
