    ...
    public static function start($timeout = 3600,$dieOnFailure = true){
		...
		if(is_file($file)){
			...
			if($d[4]){
				...
				if($dieOnFailure){ if(Cron::$output) echo "exitting process already running, finish required:: ".$d[0]." : $file\n"; exit; }
				return 3;
			}
			...
			if(!$res){
				...
				if($dieOnFailure){ if(Cron::$output) echo "exitting process already running:: ".$d[0]." : $file\n"; exit; }
				return 2;
			}else{
				if($dieOnFailure) {
					Cron::kill();
