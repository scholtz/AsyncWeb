...
class MysqlServer extends \AsyncWeb\DB\DBServer {
	...
	public function error($showquery = false){
		...
		$mes = @mysql_error($this->link);
		if($showquery || MysqlServer::$showlastquery){
			$mes .= $this->lastquery;
		}
		if(isset($mes) && $mes != ""){
   ...
   "MysqlServerError","Bola zobrazena chybova hlaska: ".addslashes($mes),ML__LOW_PRIORITY
	...
	public static  function myAddSlashes($string){
		...
		return MysqlServer::mysql_escape_mimic(stripslashes($string));



		if(mysql_real_escape_string(stripslashes($string)) == $string){
			return $string;
		}else{
	...
	public function updateSimple($table,$id,$data,$conf=array(),$insert_new = false){
		...
		if(strpos($table,".") !== false){
			$ta = explode(".",$table);
			$t = "`".MysqlServer::myAddSlashes($ta[0])."`.`".MysqlServer::myAddSlashes($ta[1])."`";
		}else{
			$t = "`".MysqlServer::myAddSlashes($table)."`";
		}
		...
		if($config !== false){
			...
			if(!isset($this->spracovanet2[$t])){
				...
				if(strpos($table,".") !== false){
					$ta = explode(".",$table);
					$t = "`".MysqlServer::myAddSlashes($ta[0])."`.`".MysqlServer::myAddSlashes($ta[1])."`";
					$schema = MysqlServer::myAddSlashes($ta[0]);
					$tab = MysqlServer::myAddSlashes($ta[1]);
				}
	...
	private function getLastUsedColumn($table){
		...
		if(strpos($table,".") !== false){
			$ta = explode(".",$table);
			$t = "`".MysqlServer::myAddSlashes($ta[0])."`.`".MysqlServer::myAddSlashes($ta[1])."`";
			$table = MysqlServer::myAddSlashes($ta[1]);
			$scheme = MysqlServer::myAddSlashes($ta[0]);
		}
