...
/**
 * Oracle Engine for AsyncWeb 
 *
 * @author Ludovit Scholtz
 * @version 1.0.4.20170212
 *
 * 1.0.0 			base db functionality
 *
 */

class Oracle11gServer extends \AsyncWeb\DB\DBServer {
	...
	public function __construct($defaultsettings = true, $server="",$login="",$pass="",$db=""){
		...
		if($defaultsettings){
			...
			if(!self::$DB && OracleServer::$DB){
				self::$SERVER = OracleServer::$SERVER;
				self::$LOGIN = OracleServer::$LOGIN;
				self::$PASS = OracleServer::$PASS;
				self::$DB = OracleServer::$DB;
				self::$PERSISTENT_CONNECTION = OracleServer::$PERSISTENT_CONNECTION;
			}
			...
			if(self::$PERSISTENT_CONNECTION){
				...
				if(!function_exists("oci_pconnect")){
					$err = "Oracle not configured properly";
					if(\AsyncWeb\DB\DB::$CONNECTED) $err = \AsyncWeb\System\Language::get($err);
					throw new \AsyncWeb\Exceptions\FatalException($err);
				}
				if(!$this->link = oci_pconnect($this->defaultLogin,$this->defaultPass,$this->defaultServer)){
				...
				if(!$this->link = oci_connect($this->defaultLogin,$this->defaultPass,$this->defaultServer)){
					$err = "Failed to connect to the database";
					if(\AsyncWeb\DB\DB::$CONNECTED) $err = \AsyncWeb\System\Language::get($err);
					throw new \AsyncWeb\Exceptions\FatalException($err);
				}
		...
		$this->query("use ".$this->defaultDB);
		self::$instance = $this;
	}
	/**
	 * tato funkcia vrati defaultnu instanciu obj. OracleServer
	...
	public static function getInstance(){
		...
		if(self::$working) throw new \AsyncWeb\Exceptions\FatalException("Oracle server loop");
		self::$working = true;

		if(self::$instance == NULL){
			try{
				self::$instance = new OracleServer();
			}catch(Exception $e){
				throw $e;
			}
		}
	...
	public function query($query,$link = null,$params = array()){
		...
		$ret = oci_parse($link,$query);
		
		foreach($params as $k=>$v){
			oci_bind_by_name($ret, $k, $v);
		}
		
		oci_execute($ret);
		$this->afrows = oci_num_rows($ret);
		...
		if($this->logqueries && $err = oci_error($this->stid)) $res = file_put_contents($this->logfile,"ERROR: ".(number_format(microtime(true)-$start,4))." ".$err."\n",FILE_APPEND);
		return $ret;
	}
	...
	public function error($showquery = false){
		...
		$mes = oci_error($this->stid);
		if($showquery || self::$showlastquery){
			$mes .= $this->lastquery;
		}
		if(isset($mes) && $mes != ""){
			\AsyncWeb\Storage\Log::log("OracleServerError","Oracle error occured: ".addslashes($mes),ML__LOW_PRIORITY);
		}
		return $mes;
	}
	public function fetch_assoc_q($query,$link=null,$params=array()){
		$res = $this->query($query,$link,$params);
		if(!$res) return false;
	...
	protected function validateColName($col){
		if(!preg_match('/^[a-zA-z\_\-]+$/', $col)){
			throw new \Exception("Invalid oracle object name");
		}
	}
	/*
	...
	public function updateOdDoTable($table,$id,$data,$od="od",$do="do",$id1="id",$id2="id2",$insert_new=false,$config=array()){
		...
		if($err = $this->error()){
			\AsyncWeb\Storage\Log::log("Oracle ERR","Error occured while inserting row into the DB.. (".addslashes($query).")");
			// zachrana polozky
	...
	public function update($table,$id2,$data=array(),$config=array(),$insert_new=false,$useOdDoSystem=true){
		...
		if($config !== false){
			...
			if(!isset($this->spracovanet2[$t])){
				...
				while($row = $this->fetch_assoc($res)){$i++;
  ...
  PRIMARY KEY  (`id`),
  ";
			if($config && array_key_exists("keys",$config))
...
";
		...
		foreach ($data as $key=>$value){
			if(!$key) continue;
			if($cols) $cols.=",";$cols .= "`$key`";
			if($value === null || $value === false){
				if($vals) $vals.=",";$vals.= "null";
			}else{
				if(@$config["cols"][$key]["compress"]){
					if($vals) $vals.=",";$vals.= "compress('".$this->myAddSlashes($value)."')";
				}elseif(@$config["cols"][$key]["type"] == "blob" || @$config["cols"][$key]["binary"] || @$config["cols"]["binary"]){
					if($vals) $vals.=",";
					if(!$value){
						$vals.="''";
					...
					if($vals) $vals.=",";$vals.= "'".$this->myAddSlashes($value)."'";
				}
			}
		}
					...
					default:
						...
						if(is_array($value)){
							...
							switch ($value["op"]){
								...
								case 'notlike':$op="not like";break;
								case 'null':$op="null";break;
							}
							if($op == "null"){
	  ...
	  if($use_od || $forceoddo){
	   $add1 = "and `$od` <= '$time'".$add1;
		...
		foreach($cols as $name => $col){
			...
			if(!is_numeric($name)){$qcols .= " as `$name`";}
		}
	...
	private function checkIndex($table,$filter=array(),$order=array(),$od="od",$do="do"){
		...
		if($table=="ZZZOracleUpdate") return true;
		if($table=="ZZZOracleTablesKeys") return true;
		...
		foreach($keys as $key){
			...
			$this->u("ZZZOracleTablesKeys",$id2,array("schema"=>$schema,"table"=>$table,"key"=>$key));
		}
		
		
		return true;
		foreach($filter as $key=>$val){
			if(strpos($val,"(")){unset($filter[$key]);continue;}
			$ar=@explode("*",$val);
			if(count($ar)>1){
				foreach($ar as $v){$filter[$v]=$v;}
			}
			$ar=@explode("/",$val);
			if(count($ar)>1){
				foreach($ar as $v){$filter[$v]=$v;}
		...
		foreach($indexes as $index){
			...
			foreach($index as $key=>$value){
				if(!array_key_exists($value,$filter)){$min = false;}
			}
			if($min){
	   $max = true;
	   foreach($filter as $key=>$value){
	   	if(is_numeric($key)) continue;
	   	if(@!array_key_exists($key,$index)){$max = false;}
	   }
			}
			if($min && $max){
				$ret = true;
		...
		if(!$ret){
			if($cols){
				$ret1 = $this->u("ZZZOracleUpdate",$id2,array("id2"=>$id2,"table"=>$table,"data"=>serialize($cols)),$config);
			}
		}
		...
		if(!$hasOd){$filter[$od] = $od;}
		if(!$hasDo){$filter[$do] = $do;}
		if(!$hasOd || !$hasDo){
	...
	public function clean($db,$dba,$table,$type="archiveObsolete",$t=1,$optimize=true,$where=array(),$dbg=false,$table2=false){
		...
		if($this->checkIndexes){
			$addLog2Lock .= ", ZZZOracleTablesKeys write";
		}
