        ...
        if ($error !== NULL) {
            if ($error["type"] == E_ERROR || $error["type"] == E_PARSE || $error["type"] == E_CORE_ERROR || $error["type"] == E_COMPILE_ERROR || $error["type"] == E_USER_ERROR) {
                if (function_exists("xdebug_is_enabled") && xdebug_is_enabled()) {
                    echo "<div>XDEBUG:";
                    echo " Called @ " . xdebug_call_file() . ":" . xdebug_call_line() . " from " . xdebug_call_function() . "</div>\n";
                ...
                echo \AsyncWeb\Exceptions\FatalException::format_error($errno, $errstr, $errfile, $errline);
                exit;
            }
        }
    }
