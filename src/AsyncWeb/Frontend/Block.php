...
class Block {
    ...
    public static function create($name = "", $tid = "", $template = null) {
        try {
            ...
            if (self::$DEBUG_TIME) {
                echo \AsyncWeb\Date\Timer1::show();
                echo "BLOCK CREATE before new block" . ($debug_iter++) . " $name $tid\n";
            }
    ...
    public function get($namespace = "") {
        ...
        if ($this->requiresAuthenticatedUser || $this->requiresAnyGroup || $this->requiresAllGroups) {
            if (self::$DEBUG_TIME) {
                echo \AsyncWeb\Date\Timer1::show();
                echo "BLOCK get before Auth::check() " . $this->name . " " . $this->tid . "\n";
            }
            if (!\AsyncWeb\Security\Auth::check()) {
            ...
            if (self::$DEBUG_TIME) {
                echo \AsyncWeb\Date\Timer1::show();
                echo "BLOCK get after Auth::check() " . $this->name . " " . $this->tid . "\n";
            }
        }
        if (self::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
        ...
        if (self::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "BLOCK get " . ($debug_iter++) . " before inclusion of other templates " . $this->name . " " . $this->tid . "\n";
        }
        $pos = 0;
        while (($pos = strpos($this->template, "{{{", $pos)) !== false) {
            $pos+= 3;
        ...
        if (self::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "BLOCK get " . ($debug_iter++) . " before filling default variables " . $this->name . " " . $this->tid . "\n";
        }
        ...
        if (self::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "BLOCK get before render" . ($debug_iter++) . " " . $this->name . " " . $this->tid . "\n";
        }
        $ret = Block::$MustacheEngine->render($this->template, $dataToRender);
        if (self::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "BLOCK get after render" . ($debug_iter++) . " " . $this->name . " " . $this->tid . "\n";
        }
        return $ret;
    }
    public function notify($namespace = "") {
        ...
        $data = json_encode($R);
        if (isset($this->clients[$namespace])) foreach ($this->clients[$namespace] as $client) {
            echo "notify: $data\n";
            if ($client) {
                $r = $client->send($data);
            }
