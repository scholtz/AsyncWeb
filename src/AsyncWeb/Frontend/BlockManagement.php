    ...
    public static function renderWeb() {
        ...
        if (BlockManagement::$defaultBlock == null) {
            ...
            if (Block::exists($index)) {
                if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
                    echo \AsyncWeb\Date\Timer1::show();
                    echo "BlockManagement:Render before creating default block " . "\n";
                }
                $def = Block::create($index);
                if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
                    echo \AsyncWeb\Date\Timer1::show();
                    echo "BlockManagement:Render after creating default block " . "\n";
                }
                BlockManagement::setDefaultBlock($def);
                if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
                    echo \AsyncWeb\Date\Timer1::show();
                    echo "BlockManagement:Render after setting default block " . "\n";
                }
            } else {
        ...
        try {
            if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
                echo \AsyncWeb\Date\Timer1::show();
                echo "BlockManagement:Render before Auth::check()" . "\n";
            }
            \AsyncWeb\Security\Auth::check();
            if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
                echo \AsyncWeb\Date\Timer1::show();
                echo "BlockManagement:Render after Auth::check()" . "\n";
            }
        }
        catch(\Exception $exc) {
