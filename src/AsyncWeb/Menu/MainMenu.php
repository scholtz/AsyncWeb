...
class MainMenu {
    ...
    public static function build($rebuild = false) {
        ...
        if (Group::is_in_group("MenuEditor")) {
            if ((null !== URLParser::v("seteditmenu"))) {
                ...
                \AsyncWeb\HTTP\Header::s("reload", array("seteditmenu" => ""));
                exit;
            }
        ...
        if (Group::is_in_group("HTMLEditor") || Group::is_in_group("PHPEditor")) {
            if ((null !== URLParser::v("seteditart"))) {
                ...
                Cache::invalidate("menu");
                \AsyncWeb\HTTP\Header::s("reload", array("seteditart" => ""));
                exit;
            }
    ...
    public static function showMenuItem(&$row, $recursive = true, $class = "menuitem", $subclass = "submenu", $showsubmenu = false, $showeditor = true, $type = "main") {
        ...
        if ($type == "main" && isset($row["submenu"]) && is_array($row["submenu"]) && $row["submenu"]) {
            $class.= " dropdown";
            $adddropdown = ' class="dropdown-toggle" data-toggle="dropdown"';
            $dropdowncaret = $DROPDOWN_CARET;
        }
        $active = "";
        ...
        if ($row["type"] == "category" && !$row["text"]) {
            $row["text"] = "?";
        }
        //if($type == "left" || $type == "nav") $row["type"] = "category";
        //var_dump($row);
        $fa = "";
        if (isset($row["fa"]) && $row["fa"]) $fa = '<i class="fa fa-' . $row["fa"] . '"></i>';
        //var_dump($row);exit;
        switch ($row["type"]) {
            case "image":
                $ret.= '<a' . $adddropdown . ' href="' . $row["path"] . '"><img src="' . $row["img"] . '" width="' . $row["imgwidth"] . '" height="' . $row["imgheight"] . '" alt="' . $row["imgalt"] . '" title="' . ($row["text"]) . '" />' . $dropdowncaret . '</a>';
            break;
            case "text":
                $ret.= '<span class="menutext">' . $fa . ($row["text"]) . '</span>';
            break;
            case "plaintext":
                $ret.= '' . $fa . ($row["text"]) . '';
            break;
            case "category":
                ...
                $ret.= '<a' . $adddropdown . ' href="' . $path . '"><span class="menutext">' . $fa . ($row["text"]) . '</span>' . $dropdowncaret . '</a>';
            break;
            case "src":
                $ret.= '<a' . $adddropdown . ' href="' . $row["path"] . '"><span class="menutext">' . $fa . $row["text"] . '</span>' . $dropdowncaret . '</a>';
            break;
        }
        $sub = "";
        $i = 0;
    ...
    public static function showTopMenu() {
        ...
        $i = 0;
        ksort(MainMenu::$menu);
        foreach (MainMenu::$menu as $k => $row) {
            $i++;
            if ($k . "" == "submenu") continue;
...
';
    ...
    public static function getCurrent($dbg = false) {
        ...
        //$dbg= true;
        if ($dbg) {
            echo Timer1::show() . "MainMenu:getCurrent:" . ($dbgi++) . ":\n";
        }
        MainMenu::build();
        if ($dbg) {
            echo Timer1::show() . "MainMenu:getCurrent:" . ($dbgi++) . ":\n";
        ...
        if ($dbg) {
            echo Timer1::show() . "MainMenu:getCurrent:gettitle:" . ($dbgi++) . ":\n";
        }
        if (isset($ret["title"])) {
        ...
        if ($ret) {
            if (!MainMenu::$builderschecked) {
                MainMenu::$builderschecked = true;
                foreach (MainMenu::$builders as $builder) {
                    $builder->check();
                }
            }
