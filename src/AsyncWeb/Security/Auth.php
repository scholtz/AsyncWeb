...
class Auth {
    ...
    public static function check($skipControllers = false) {
        ...
        if (!$skipControllers && $ret) {
            $ret = (true === Auth::checkControllers());
        } // if any controller fails, check is false
        Auth::$CHECKING = false;
        if (!$skipControllers) {
            Auth::$CHECKED = $ret;
        }
    ...
    public static function checkControllers() {
        ...
        if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
            ...
            echo "Auth::checkControllers init" . "\n";
        }
        ...
        if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "Auth::checkControllers finished" . "\n";
        }
    ...
    public static function auth(Array $data, AuthService $service) {
        if (!isset(Auth::$services[$service->SERVICE_ID() ])) {
            throw new \AsyncWeb\Exceptions\SecurityException("Service provider '" . $service->SERVICE_ID() . "' is not registered! 0x9319520");
        }
        $auth = Session::get(Auth::$SESS_CHAIN_NAME);
    ...
    public static function userId() {
        if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "Auth::userID on start" . "\n";
        }
        $auth = Session::get(Auth::$SESS_CHAIN_NAME);
        if (\AsyncWeb\Frontend\Block::$DEBUG_TIME) {
            echo \AsyncWeb\Date\Timer1::show();
            echo "Auth::userID after session init" . "\n";
        }
        if ($auth && is_array($auth)) {
            $data = array_pop($auth);
