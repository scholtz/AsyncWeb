...
class Session {
    ...
    public static function init($force = false) {
        ...
        if (!isset($_COOKIE["PHPSESSID"]) && $force == false) {
            return;
        }
        if (!Session::$inst) {
    ...
    public static function get($var) {
        ...
        //   var_dump($_SERVER);
        if (Session::$CHECK_IP) {
            if (@$_SESSION["SESSION_STEALING__IP"] != @$_SERVER['REMOTE_ADDR'] || @$_SESSION["SESSION_STEALING__UA"] != @$_SERVER['HTTP_USER_AGENT'] || @$_SESSION["SESSION_STEALING__GI"] != @$_SERVER['GATEWAY_INTERFACE'] || @$_SESSION["SESSION_STEALING__FW"] != @$_SERVER['HTTP_X_FORWARDED_FOR']) {
                \AsyncWeb\Storage\Log::log("SESSION_module", "SESSION STEALING - 
...
for: " . @$_SESSION['SESSION_STEALING__FW'], ML__TOP_PRIORITY);
                session_destroy();
                Session::set_cookie_params();
                session_start();
            ...
            if (@$_SESSION["SESSION_STEALING__UA"] != @$_SERVER['HTTP_USER_AGENT']) {
                \AsyncWeb\Storage\Log::log("SESSION_module", "SESSION STEALING - 
ip:" . @$_SERVER['REMOTE_ADDR'] . ";
ua:" . @$_SERVER['HTTP_USER_AGENT'] . ";
via:" . @$_SERVER['HTTP_VIA'] . ";
...
for: " . @$_SESSION['SESSION_STEALING__FW'], ML__TOP_PRIORITY);
    ...
    private static function check_timeout() {
        ...
        $time = $ctime;
        if (isset($_SESSION)) {
            if (array_key_exists(__SESSION_time_var, $_SESSION)) {
                $time = $_SESSION[__SESSION_time_var];
            }
        }
    ...
    public static function check_checksum() {
        ...
        if ((Session::$checksum == null && (Session::make_checksum() == @$_SESSION[__SESSION_checksum_var])) || (Session::$checksum != null && (Session::$checksum == @$_SESSION[__SESSION_checksum_var]))) {
            return true;
        } else {
            //  		echo "b"; exit;
