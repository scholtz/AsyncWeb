...
//   	 return "?lang=".$name;
 ...
 public static function m($tmpl,$var){
	 \AsyncWeb\Frontend\URLParser::merge(
		array(
		"tmpl"=>$tmpl,
		"var"=>$var));
 }
 ...
 public static function make($params=array(),$moveparams=true,$uri=null,$paramsAreSafe=false,$getIsSafe=false,$js=false){
  ...
  foreach($params as $key=>$value){
    if(!$paramsAreSafe){$key=htmlentities($key);$value=htmlentities($value);}
	//echo $key.":".$usedkeys[$key]."\n";
    if(@$usedkeys[$key]) continue;
	$usedkeys[$key] = true;
	if($value){
     if(!$par && !$uricontainsq){$par = "?";}else{$par.=$amp;}
	 $par.=$key."=".$value;
	}
  }
  ...
  if($moveparams){
   foreach($_GET as $key=>$value){
    if(!$getIsSafe){$key=htmlentities($key);$value=htmlentities($value);}
	//echo $key.":".$usedkeys[$key]."\n";
    if(@$usedkeys[$key]) continue;
	$usedkeys[$key] = true;
    if(!$par && !$uricontainsq){$par = "?";}else{$par.=$amp;}
