...
class Messages {
    ...
    private function __construct() {
        if ($this->err == "" && \AsyncWeb\Storage\Session::get($this->conf_ses_save_err)) {
            $this->err = \AsyncWeb\Storage\Session::get($this->conf_ses_save_err);
        }
        if ($this->warn == "" && \AsyncWeb\Storage\Session::get($this->conf_ses_save_warn)) {
            $this->warn = \AsyncWeb\Storage\Session::get($this->conf_ses_save_warn);
        }
        if ($this->msg == "" && \AsyncWeb\Storage\Session::get($this->conf_ses_save_msg)) {
            $this->msg = \AsyncWeb\Storage\Session::get($this->conf_ses_save_msg);
        }
        if ($this->alert == "" && \AsyncWeb\Storage\Session::get($this->conf_ses_save_alert)) {
            $this->alert = \AsyncWeb\Storage\Session::get($this->conf_ses_save_alert);
        }
    }
    /**
     * Instance
    ...
    public function error($mes, $alert = false, $finish = false) {
        \AsyncWeb\Storage\Log::log("MessagesError", "Bola zobrazena chybova hlaska: " . addslashes($mes), ML__LOW_PRIORITY);
    ...
    public function warning($mes, $alert = true) {
        \AsyncWeb\Storage\Log::log("MessagesWarning", "Bola zobrazena varovacia hlaska: " . addslashes($mes), ML__LOW_PRIORITY);
