...
class Validate{
	...
	public static function check($type,&$data){
		...
		switch(strtolower($type)){
			...
			break;
			case '': return true;
			case '1': if(preg_match("\d+",$data)){return true;}else{return false;} break;;
			case '2': if(ereg("[<>\"'!@#\$%^&*;\\\`\]",$data)){ return false;}else{return true;} break;;
			case '3': if(ereg("[<>\"']",$data)){ return false;}else{return true;} break;;
			case '4': if(ereg("[\"']",$data)){ return false;}else{return true;} break;;
			case '5': if(ereg("[<>]",$data)){ return false;}else{return true;} break;;
			case 'tel': if(ereg("^[[:digit:] +/][[:digit:] +/]*$",$data)){ return true;}else{return false;} break;;
