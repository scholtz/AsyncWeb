...
class SubmitResetCancel extends \AsyncWeb\View\FormItemInstance {
    public function TagName() {
        return "submitResetCancel";
    }
    public function InsertForm($SubmittedValue = null) {
        $Template = "View_FormItem_Submit";
        ...
        $CancelPath = Path::make(array($this->data["uid"] . "___CANCEL" => "1"));
        $TextCancel = "Cancel";
        if (isset($this->item["texts"]["cancel"])) $TextCancel = Language::get($this->item["texts"]["cancel"]);
        $Name = $ID;
        if (isset($this->item["data"]["var"]) && $this->item["data"]["var"]) $Name = $this->item["data"]["var"];
        $DataValidation = $this->DataValidation();
        return \AsyncWeb\Text\Template::loadTemplate($Template, array("LabelText" => $LabelText, "Prepend" => $Prepend, "After" => $After, "ID" => $ID, "AddClass" => $AddClass, "Name" => $Name, "Title" => $Help, "Placeholder" => $Placeholder, "DataContent" => $Help, "Value" => $Value, "ShowReset" => true, "ResetValue" => $ResetValue, "ShowCancel" => true, "CancelPath" => $CancelPath, "TextCancel" => $TextCancel, "BT_SIZE" => $this->item["BT_SIZE"], "BT_WIDTH_OF_LABEL" => $this->item["BT_WIDTH_OF_LABEL"], "BT_WIDTH_9" => $this->item["BT_WIDTH_9"], "BT_WIDTH_10" => $this->item["BT_WIDTH_10"], "DataValidation" => $DataValidation,), false, false);
    }
