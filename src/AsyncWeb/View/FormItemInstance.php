...
class FormItemInstance implements \AsyncWeb\View\FormItemInterface{
	...
	protected function DataValidation(){
		...
		if(isset($this->item["data"]["validation"])){
			...
			$modules = array();
			if($MinLength > 0 ){$modules["Required"] = true;}
			
			$MinLength = null;		if(isset($this->item["data"]["minlength"]) && $this->item["data"]["minlength"]) $MinLength = $this->item["data"]["minlength"];
			$MaxLength = null;		if(isset($this->item["data"]["maxlength"]) && $this->item["data"]["maxlength"]) $MaxLength = $this->item["data"]["maxlength"];
			
			$Min = null;			if(isset($this->item["data"]["minnum"]) && $this->item["data"]["minnum"]) $Min = $this->item["data"]["minnum"];
			$Max = null;			if(isset($this->item["data"]["maxnum"]) && $this->item["data"]["maxnum"]) $Max = $this->item["data"]["maxnum"];
			$Step = null;			if(isset($this->item["data"]["step"]) && $this->item["data"]["step"]) $Step = $this->item["data"]["step"];
			
			if(isset($this->item["data"]["datatype"]) && ($this->item["data"]["datatype"] == "email")){
				$modules["email"] = true;
			}else
			if(isset($this->item["data"]["datatype"]) && ($this->item["data"]["datatype"] == "url")){
				$modules["url"] = true;
			}else
			...
			if(isset($Min) || isset($Max) || isset($Step)){
				...
				if($Min < 0 || $Max < 0) {if(isset($ret["Allowing"])) $ret["Allowing"] .= ",";$ret["Allowing"].="negative";}
			}
