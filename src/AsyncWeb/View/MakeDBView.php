...
class MakeDBView{
	...
	private static function makeTableView($data){
		...
		foreach ($data["col"] as $col=>$info){
			...
			$display = false;
			$exportable = false;
			$usg="DBVs";if(isset($info["usage"]) && ((isset($info["usage"][$usg]) && $info["usage"][$usg]) || in_array($usg,$info["usage"]))){$display=true;}else{}
			$usg="DBVe";if(isset($info["usage"]) && ((isset($info["usage"][$usg]) && $info["usage"][$usg]) || in_array($usg,$info["usage"]))){$exportable=true;}else{}
	...
	private static function makeCols(&$data){
		...
		foreach($data["col"] as $col=>$info){
			...
			$display = false;
			$export = false;
			$usg = "DBVs";if(isset($info["usage"]) && ((isset($info["usage"][$usg]) && $info["usage"][$usg]) || in_array($usg,$info["usage"]))){$display=true;}else{$display=false;}
			$usg = "DBVe";if(isset($info["usage"]) && ((isset($info["usage"][$usg]) && $info["usage"][$usg]) || in_array($usg,$info["usage"]))){$export = true;}else{$export=false;}
