    ...
    private static function makeExportCSV($data) {
        ...
        while ($row = DB::fetch_assoc($res)) {
            ...
            foreach ($data["col"] as $col => $prop) {
                $usg = "DBVs";
                if (isset($prop["usage"]) && ((isset($prop["usage"][$usg]) && $prop["usage"][$usg]) || in_array($usg, $prop["usage"]))) {
                } else {
                    continue;
                }
    ...
    private static function checkFilterFormChange($data) {
        ...
        if (@$_REQUEST["filter_" . $data["uid"]]) {
            ...
            foreach ($_REQUEST["col"] as $key => $value) {
                ...
                $filter[$key] = array("col" => @$_REQUEST["col"][$key], "operator" => @$_REQUEST["operator"][$key], "value" => @$_REQUEST["value"][$key], "operator2" => @$_REQUEST["operator2"][$key],);
            }
            \AsyncWeb\Storage\Session::set("MDBV_filter_" . $data["uid"], $filter);
...
';
    ...
    private static function makeColSelect($data, $default = "") {
        ...
        foreach ($data["col"] as $col => $info) {
            ...
            $ret.= '<option value="' . $col . '"';
            if ($default == $col) {
                $ret.= ' selected="selected"';
            }
            $ret.= '>' . $info["name"] . '</option>';
        }
        $ret.= '</select>';
        return $ret;
    }
    private static function makeOpSelect($default = "like") {
        ...
        $ret.= '>' . Language::get("contains") . '</option>
		<option value="notlike"';
        if ($default == "notlike") {
            $ret.= ' selected="selected"';
        }
        $ret.= '>' . Language::get("deosnotcontains") . '</option>
		<option value="null"';
        if ($default == "null") {
            $ret.= ' selected="selected"';
        }
        $ret.= '>' . Language::get("isempty") . '</option>
		</select>';
        return $ret;
    }
    private static function makeOp2Select($default = "and") {
        ...
        $ret.= '>' . Language::get("And") . '</option>
		<option value="or"';
        if ($default == "or") {
            $ret.= ' selected="selected"';
        }
        $ret.= '>' . Language::get("Or") . '</option>
		<option value="andlz"';
        if ($default == "andlz") {
            $ret.= ' selected="selected"';
        }
    ...
    private static function makeTableView($data) {
        ...
        if (array_key_exists("nadpis", $data)) $ret.= '<div class="data_table_' . $prefix . '_header">' . $data["nadpis"] . '</div>';
        if (!isset($data["rowwhere"])) {
            if ($iter->getPagesCount($count) > 2) $ret.= $iter->show_bar($count, 5, array(), "top_iter");
        }
		...
		<tr>';
            ...
            while ($row = DB::fetch_assoc($res)) {
                ...
                foreach ($data["col"] as $col => $prop) {
                    ...
                    if (isset($data["allowUpdate"]) && $data["allowUpdate"] && !$update) {
                        ...
                        if (\AsyncWeb\IO\File::exists("img/update.png")) {
                            $ret.= '<img src="/img/update.png" width="20" height="20" alt="' . Language::get("update") . '" />';
                        } else {
                            $ret.= Language::get("update");
                        }
                        $ret.= '</a></td>';
                    }
                    if (isset($data["allowDelete"]) && $data["allowDelete"] && !$delete) {
                        ...
                        if (\AsyncWeb\IO\File::exists("img/delete.png")) {
                            $ret.= '<img src="/img/delete.png" width="20" height="20" alt="' . Language::get("delete") . '" />';
                        } else {
                            $ret.= Language::get("delete");
                        }
                        $ret.= '</a></td>';
                    }
                    $ret.= '</tr>';
            ...
            $ret.= '</tbody></table>';
            if (!isset($data["rowwhere"])) {
                if ($iter->getPagesCount($count) > 2) $ret.= $iter->show_bar($count, 5, array(), "bottom_iter");
            }
            return $ret;
        }
        private static function makeVerticalTableView($data) {
            ...
            $uid = $data["uid"];
            if (isset($data["name"]) && $data["name"]) $nadpis = Language::get($data["name"]);
            if (array_key_exists("nadpis", $data) && $data["nadpis"]) $ret.= '<div class="data_table_' . $prefix . '_header">' . $data["nadpis"] . '</div>';
            if (array_key_exists("popis", $data) && $data["popis"]) $ret.= '<div class="data_table_' . $prefix . '_description">' . Language::get($data["popis"]) . '</div>';
            $ret.= '<table id="data_table_' . $data["uid"] . '" class="data_table_' . $prefix;
            if (isset($data["bootstrap"])) $ret.= " table";
            $ret.= '">';
            $ret.= '<tbody>';
