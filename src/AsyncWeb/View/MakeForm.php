...
class MakeForm {
    ...
    public function __construct($data = array(), $merged = null) {
        ...
        if (isset($this->data["tableN2N"]) && isset($this->data["tableN2Ncol"]) && isset($this->data["tableN2Nvalue"]) && isset($this->data["tableN2Ncol2"])) {
            MakeForm::$N2NData = array("table" => @$data["table"], "tableN2N" => @$data["tableN2N"], "tableN2Ncol" => @$data["tableN2Ncol"], "tableN2Nvalue" => @$data["tableN2Nvalue"], "tableN2Ncol2" => @$data["tableN2Ncol2"], "tableN2Ncol2Textcol" => @$data["tableN2Ncol2Textcol"], "tableN2Ncol2Where" => @$data["tableN2Ncol2Where"]);
    ...
    private function getExceptionText(&$item, $exception) {
        ...
        public function show($what = "ALL", $show_results = true) {
            ...
            if (@$this->data["expectRight"]) if (!Group::is_in_group($this->data["expectRight"])) return;
  ...
  ';
        ...
        private function inWhere($col) {
            $ret = array();
            if (isset($this->data["where"])) foreach ($this->data["where"] as $k => $v) {
                if (is_array($v)) {
        ...
        private function insertForm() {
            ...
            $EncType = false;
            if (isset($this->data["enctype"]) && $this->data["enctype"]) $EncType = $this->data["enctype"];
            $OnSubmit = false;
            if (isset($this->data["form"]["onInsertSubmit"]) && $this->data["form"]["onInsertSubmit"]) $OnSubmit = $this->data["form"]["onInsertSubmit"];
            $Template = "View_InsertForm";
            if (isset($this->data["template"]["insert"]) && $this->data["template"]["insert"]) $Template = Language::get($this->data["template"]["insert"]);
            return \AsyncWeb\Text\Template::loadTemplate($Template, array("HeaderText" => $text, "HTML" => $ret, "Focus" => $Focus, "ID" => $this->data["uid"], "Merged" => $this->merged, "SubmitURL" => $SubmitURL, "OnSubmit" => $OnSubmit, "EncType" => $EncType), false, false);
        }
        private function makeN2NInsertCols() {
            $cols = array(array("name" => Language::get("Select existing or new item"), "form" => array("type" => "selectDB"), "data" => array("col" => $this->data["tableN2Ncol2"], "allowNull" => true, "fromTable" => $this->data["table"], "fromColumn" => $this->data["tableN2Ncol2Textcol"], "dictionary" => true, "where" => @$this->data["tableN2Ncol2Where"]), "usage" => array("MFi"), "texts" => array("nullValue" => "New item", "allowNull" => true)));
        ...
        private function makeSelect() {
            ...
            while ($row = DB::fetch_assoc($res)) {
                ...
                if (isset($this->data["secondaryData"]["data"])) foreach ($this->data["secondaryData"]["data"] as $data) {
                    if ($data["type"] == "text") {
                        $row[$col].= $data["text"];
        ...
        private function update2Form() {
            ...
            if (isset($this->data["template"]["update"]) && $this->data["template"]["update"]) $Template = Language::get($this->data["template"]["update"]);
            $ShowUp = false;
            if (isset($this->data["showUp"]) && $this->data["showUp"]) $ShowUp = true;
            $ShowUpURL = $ShowUpText = false;
            if ($ShowUp) {
                $ShowUpURL = Path::make(array("REMOVE_VARIABLES" => "1"));
                $ShowUpText = Language::get("Show data");
            }
            return \AsyncWeb\Text\Template::loadTemplate($Template, array("HeaderText" => $text, "HTML" => $ret, "Focus" => $Focus, "ID" => $this->data["uid"], "Merged" => $this->merged, "SubmitURL" => $SubmitURL, "OnSubmit" => $OnSubmit, "EncType" => $EncType, "OriginalRecordID" => $OriginalRecordID, "ShowUp" => $ShowUp, "ShowUpURL" => $ShowUpURL, "ShowUpText" => $ShowUpText), false, false);
        }
