...
class MakeForm
{
    ...
    public function __construct($data = array(), $merged = null)
    {
        ...
        if (isset($this->data["tableN2N"]) && isset($this->data["tableN2Ncol"]) && isset($this->data["tableN2Nvalue"]) && isset($this->data["tableN2Ncol2"])) {
            ...
            $row               = DB::qbr($this->data["tableN2N"], array(
                "where" => array(
                    array(
            ...
            if ($row) {
                ...
                $res = DB::qb($data["tableN2N"], array(
                    "where" => array(
                        $this->data["tableN2Ncol"] => $this->data["tableN2Nvalue"]
                    ),
                    "cols" => $this->data["tableN2Ncol2"]
                ));
                $i   = 0;
                if (DB::num_rows($res) > 0) {
                    ...
                    while ($row = DB::f($res)) {
                        ...
                        $this->data["where"][] = array(
                            "col" => "id2",
                            "op" => "eq",
                            "value" => $row[$this->data["tableN2Ncol2"]]
                        );
                    }
                    $this->data["where"][] = array(
                        "col" => "-)"
        ...
        foreach ($this->data["col"] as $k => $col) {
            ...
            if (!isset($col["data"]["col"]) && !is_numeric($k))
                $this->data["col"][$k]["data"]["col"] = $k;
        ...
        if (isset($this->data["iter_per_page"]))
            $this->data["iter"]["per_page"] = $this->data["iter_per_page"];
        ...
        if (!$merged && isset($this->data["col"])) {
            ...
            foreach ($this->data["col"] as $col) {
                if (!isset($col["form"]))
                    continue;
                if ($col["form"]["type"] == "submitReset" || $col["form"]["type"] == "submit" || $col["form"]["type"] == "submitResetCancel")
                    $submit = true;
            }
            if (!$submit) {
                $this->data["col"][] = array(
        ...
        if ((isset($this->data["allowInsert"]) && $this->data["allowInsert"]) || (isset($this->data["allowUpdate"]) && $this->data["allowUpdate"]) || (isset($this->data["allowDelete"]) && $this->data["allowDelete"])) {
    ...
    public static function N2NBeforeDelete($r)
    {
        ...
        if (isset(MakeForm::$N2NData["tableN2N"]) && isset(MakeForm::$N2NData["tableN2Ncol"]) && isset(MakeForm::$N2NData["tableN2Nvalue"]) && isset(MakeForm::$N2NData["tableN2Ncol2"])) {
            ...
            DB::delete(MakeForm::$N2NData["tableN2N"], array(
                MakeForm::$N2NData["tableN2Ncol"] => MakeForm::$N2NData["tableN2Nvalue"],
                MakeForm::$N2NData["tableN2Ncol2"] => $row["id2"]
            ));
            // if it is last occurance in n2n table, delete from source table
            $nrow = DB::qbr(MakeForm::$N2NData["tableN2N"], array(
                "cols" => array(
                    "c" => "count(`id2`)"
                ),
                "where" => array(
            ...
            \AsyncWeb\HTTP\Header::s("location", array(
                "REMOVE_VARIABLES" => "1"
            ));
            exit;
            return false;
    ...
    {
        $row = $r["row"];
        DB::u(MakeForm::$N2NData["tableN2N"], MakeForm::$N2NData["id2"], array(
            MakeForm::$N2NData["tableN2Ncol2"] => $row["id2"]
        ));
    }
    /**
    ...
    public function check_update()
    {
        ...
        if ($this->exception) {
            ...
            Header::s("reload", array(
                $this->data["uid"] . "___INSERT" => "",
                "insert_data_" . $this->data["uid"] => "",
                $this->data["uid"] . "___CANCEL" => "",
                $this->data["uid"] . "___ID" => "",
                $this->data["uid"] . "___UPDATE2" => "",
                $this->data["uid"] . "___UPDATE1" => "",
                $this->data["uid"] . "___ID" => "",
                $this->data["uid"] . "___DELETE" => ""
            ));
            exit;
        }
        return false;
    ...
    private function getExceptionText(&$item, $exception)
    {
        ...
        switch ($exception) {
            case 'dataTypeException':
                if (isset($item["texts"]["exception-$exception"]) && $item["texts"]["exception-$exception"])
                    return $this->getText($item["texts"]["exception-$exception"]);
                if (isset($item["texts"]["exceptions"]["$exception"]))
                    return $this->getText($item["texts"]["exceptions"]["$exception"]);
                if (isset($this->data["$exception"]))
                    return $this->getText($this->data["$exception"]);
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Wrong data type has been supplied");
                break;
            case 'minLengthException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Input is too short");
                break;
            case 'maxLengthException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Input is too long");
                break;
            case 'uniqueException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Value must be unique. This value is already in the DB.");
                break;
            case 'fileExistsException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("File with the same name already exists");
                break;
            case 'fileNotAllowed':
                if (isset($item["texts"]["exception-$exception"]) && $item["texts"]["exception-$exception"])
                    return $this->getText($item["texts"]["exception-$exception"]);
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("File cannot be processed because of security reasons");
                break;
            case 'errorWhileMovingFile':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Error occured while copying file");
                break;
            case 'minNumException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Number is too low");
                break;
            case 'maxNumException':
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Number is too high");
                break;
            case "enumTypeException":
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Wrong value has been entered");;
                break;
            case "captchaTypeException":
                ...
                if (!$text)
                    $text = $this->getText("MF_$exception");
                if (!$text || $text == "MF_$exception")
                    $text = Language::get("Captcha is not correct");;
                break;
        }
        return $text;
    }
    ...
    private function checkRightDataFormat(&$item, $name1, $update_ignore = false, $form_name = "")
    {
        ...
        if (isset($item["data"]["unique"]) && $item["data"]["unique"]) {
            $row = DB::gr($this->data["table"], array(
                $item["data"]["col"] => URLParser::v($item)
            ));
            if ($row) {
    ...
    private function filters($value, $type = null, $safe = true)
    {
        ...
        if (!$safe) {
            if (($val = $this->execute($value)) !== false) {
                return $val;
            }
        }
        return $value;
    ...
    private function checkCancel() // for N2N tables if the item should be canceled
    {
        ...
        if (!isset($this->data["allowInsert"]))
            return false;
        if (!(null !== URLParser::v($this->data["uid"] . "___CANCEL")))
            return false;
        ...
        if (isset($this->data["rights"])) {
            if (isset($this->data["rights"]["insert"]) && $this->data["rights"]["insert"]) { // ak sa vyzaduju prava na vkladanie, tak ich over
                if (Group::exists($this->data["rights"]["insert"])) { // ak existuje dane id skupiny
                    if (!Group::isInGroupId($this->data["rights"]["insert"]))
                        return false;
                } else { // inak existuje nazov skupiny
                    if (!Group::userInGroup($this->data["rights"]["insert"]))
                        return false;
                }
            } else {
                if (@$this->data["rights"]["insert"])
                    return false;
            }
        }
        $del = array(
    ...
    private function checkInsert()
    {
        if (!isset($this->data["allowInsert"]))
            return false;
        if (!(null !== URLParser::v($this->data["uid"] . "___INSERT")))
            return false;
        ...
        foreach ($this->data["col"] as $colname => $item) {
            ...
            if (isset($item["data"]["type"]))
                $item["data"]["datatype"] = $item["data"]["type"];
            $usg = "MFi";
            if (isset($item["usage"]) && ((isset($item["usage"][$usg]) && $item["usage"][$usg]) || in_array($usg, $item["usage"]))) {
            } else {
                continue;
            }
            
            if (is_numeric($colname) && !isset($item["data"]["col"]))
                continue;
            
            if (isset($item["data"]["col"]))
                $colname = $item["data"]["col"];
            $n = $formName . "_" . $colname;
            
            if (isset($item["data"]["var"]))
                $n = $item["data"]["var"];
            ...
            if (!(null !== URLParser::v($name1)) && (null !== URLParser::v($n)))
                $colValue = URLParser::v($n);
            
            if ($item["form"]["type"] == "part")
                continue;
            ...
            if (!isset($item["FormItemInstance"])) {
                ...
                if (!isset(self::$ItemsMap[$item["form"]["type"]])) {
                    throw new \Exception(Language::get("Form type %formtype% has not been found!", array(
    ...
    private function doInsert()
    {
        if (!isset($this->data["allowInsert"]))
            return false;
        if (!(null !== URLParser::v($this->data["uid"] . "___INSERT")) || !URLParser::v($this->data["uid"] . "___INSERT"))
            return false;
        if (!isset($this->data["table"]))
            return $this->merged;
        ...
        if ($res) {
            ...
            if (!$this->merged) {
                ...
                if (!$text || $text == "insertSucces" || $text == "insertSuccess")
                    $text = Language::get("New item has been successfully inserted");
    ...
    {
        if (!isset($this->data["allowUpdate"]))
            return false;
        if (!(null !== URLParser::v($this->data["uid"] . "___UPDATE2")) || !URLParser::v($this->data["uid"] . "___UPDATE2"))
            return false;
        if (!isset($this->data["table"]))
            return $this->merged;
        ...
        if (isset($this->data["rights"])) {
            if (isset($this->data["rights"]["update"]) && $this->data["rights"]["update"]) { // ak sa vyzaduju prava na vkladanie, tak ich over
                if (Group::exists($this->data["rights"]["update"])) { // ak existuje dane id skupiny
                    if (!Group::isInGroupId($this->data["rights"]["update"]))
                        return false;
                } else { // inak existuje nazov skupiny
                    if (!Group::userInGroup($this->data["rights"]["update"]))
                        return false;
                }
            } else {
                if (@$this->data["rights"]["update"])
                    return false;
            }
        }
        $cols     = array();
        ...
        if (isset($this->data["execute"])) {
            if (isset($this->data["execute"]["beforeUpdate"])) {
                try {
                    ...
                    if (!$this->execute($this->data["execute"]["beforeUpdate"], array(
                        "r" => $row
                    ))) {
                        return false;
                    }
    ...
    private function doUpdate()
    {
        if (!isset($this->data["allowUpdate"]))
            return false;
        if (!(null !== URLParser::v($this->data["uid"] . "___UPDATE2")))
            return false;
        if (!isset($this->data["table"]))
            return $this->merged;
        ...
        if (isset($this->data["execute"])) {
            if (isset($this->data["execute"]["onUpdate"])) {
                $ret = $this->execute($this->data["execute"]["onUpdate"], array(
                    "old" => $old_row,
                    "new" => $new_row
                ));
            }
        ...
        if (!$this->merged) {
            ...
            if (!$text || $text == "updateSucces" || $text == "updateSuccess")
                $text = Language::get("Item has been successfully updated");
            Messages::getInstance()->mes($text); //
            Header::s("reload", array(
                $this->data["uid"] . "___ID" => "",
                $this->data["uid"] . "___UPDATE2" => "",
    ...
    {
        if (!isset($this->data["allowDelete"]))
            return false;
        
        if (!(null !== URLParser::v($this->data["uid"] . "___DELETE")) || !URLParser::v($this->data["uid"] . "___DELETE"))
            return false;
        if (!isset($this->data["table"]))
            return $this->merged;
        ...
        if (isset($this->data["rights"])) {
            if (isset($this->data["rights"]["delete"]) && $this->data["rights"]["delete"]) { // ak sa vyzaduju prava na vkladanie, tak ich over
                if (Group::exists($this->data["rights"]["delete"])) { // ak existuje dane id skupiny
                    if (!Group::isInGroupId($this->data["rights"]["delete"]))
                        return false;
                } else { // inak existuje nazov skupiny
                    if (!Group::userInGroup($this->data["rights"]["delete"]))
                        return false;
                }
            } else {
        ...
        if (isset($this->data["execute"])) {
            if (isset($this->data["execute"]["beforeDelete"])) {
                try {
                    if (!$this->execute($this->data["execute"]["beforeDelete"], array(
                        "r" => $row
                    ))) {
                        return false;
                    }
    ...
    private function doDelete()
    {
        ...
        if (!(null !== URLParser::v($this->data["uid"] . "___DELETE")))
            return false;
        if (!isset($this->data["table"]))
            return $this->merged;
        $table = $this->data["table"];
        ...
        if (DB::affected_rows() > 0) {
            ...
            if (isset($this->data["execute"])) {
                if (isset($this->data["execute"]["onDelete"])) {
                    $this->execute($this->data["execute"]["onDelete"], array(
                        "row" => $row,
                        "old" => $row
                    ));
                }
            ...
            if (!$this->merged) {
                ...
                if (!$text || $text == "deleteSucces")
                    $text = Language::get("Deletion has been successfully commited");
            ...
            if (!$this->merged) {
                ...
                if (!$text || $text == "deleteNotSucces" || $text == "deleteNotSuccess")
                    $text = Language::get("Error occured while deleting the item");
    ...
    public function show($what = "ALL", $show_results = true)
    {
        ...
        $ret = "";
        if ($show_results)
            $ret .= $this->show_results();
        if (@$this->data["expectRight"])
            if (!Group::is_in_group($this->data["expectRight"]))
                return;
  ...
  ';
    ...
    private function insertForm()
    {
        ...
        if (isset($this->data["rights"])) {
            if (isset($this->data["rights"]["insert"]) && $this->data["rights"]["insert"]) { // ak sa vyzaduju prava na vkladanie, tak ich over
                if (Group::exists($this->data["rights"]["insert"])) { // ak existuje dane id skupiny
                    ...
                    if (!Group::is_in_group($this->data["rights"]["insert"]))
                        return false;
                }
            } else {
                if (@$this->data["rights"]["insert"])
        ...
        foreach ($this->data["col"] as $colname => $item) {
            ...
            if (!isset($item["BT_SIZE"]))
                $item["BT_SIZE"] = $this->BT_SIZE;
            if (!isset($item["BT_WIDTH_OF_LABEL"]))
                $item["BT_WIDTH_OF_LABEL"] = $this->BT_WIDTH_OF_LABEL;
            if (!isset($item["BT_WIDTH_9"]))
                $item["BT_WIDTH_9"] = 12 - 1 - $this->BT_WIDTH_OF_LABEL;
            if (!isset($item["BT_WIDTH_10"]))
                $item["BT_WIDTH_10"] = 12 - $this->BT_WIDTH_OF_LABEL;
        ...
        $ID        = $this->data["uid"];
        $SubmitURL = Path::make(array(
            $ID . "___INSERT" => 1
        ));
        $EncType   = false;
        if (isset($this->data["enctype"]) && $this->data["enctype"])
            $EncType = $this->data["enctype"];
        $OnSubmit = false;
        if (isset($this->data["form"]["onInsertSubmit"]) && $this->data["form"]["onInsertSubmit"])
            $OnSubmit = $this->data["form"]["onInsertSubmit"];
        $Template = "View_InsertForm";
        if (isset($this->data["template"]["insert"]) && $this->data["template"]["insert"])
            $Template = Language::get($this->data["template"]["insert"]);
    ...
    private function makeN2NInsertCols()
    {
        ...
        $cols   = array(
            array(
                "name" => Language::get("Select existing or new item"),
                "form" => array(
                    "type" => "selectDB"
                ),
                "data" => array(
    ...
    private function makeSelect()
    {
        ...
        $res = DB::g($this->data["table"], $this->where, $offset = null, $count = null, $order = array(
            $col => "asc"
        ));
                ...
                foreach ($this->data["secondaryData"]["data"] as $data) {
                    if ($data["type"] == "text") {
                        ...
                        if ($data["table"]) {
                            ...
                            $row2 = DB::gr($data["table"], array(
                                $colId => $row[$data["colIn"]]
                            ), array(), $cols = array(
                                $data["col"]
                            ), $groupby = array(), $having = array(), $offset = 0, $time = $row["od"]);
    ...
    private function update1Form()
    {
        ...
        if ($this->merged) {
            ...
            $ret .= "<form method=\"post\" action=\"" . Path::make(array(
                $this->data["uid"] . "___UPDATE1" => "1"
            )) . "\">\n";
        }
        $ret .= "<table>";
        $ret .= "<tr><td colspan=\"3\">";
    ...
    private function update2Form()
    {
        ...
        $Template = "View_UpdateForm";
        if (isset($this->data["template"]["update"]) && $this->data["template"]["update"])
            $Template = Language::get($this->data["template"]["update"]);
        
        $ShowUp = false;
        if (isset($this->data["showUp"]) && $this->data["showUp"])
            $ShowUp = true;
        $ShowUpURL = $ShowUpText = false;
        if ($ShowUp) {
            $ShowUpURL  = Path::make(array(
                "REMOVE_VARIABLES" => "1"
            ));
    ...
    private function deleteForm()
    {
        ...
        if (!$text)
            $text = "Naozaj chcete zrušiť položku?";
        ...
        if ($this->merged) {
            ...
            $ret .= "<form method=\"post\" action=\"" . Path::make(array(
                $name . "___DELETE" => "1"
            )) . "\" onsubmit=\"confirm('" . $confirm_text . "')?ret=true:ret=false;return ret;\">\n";
        }
        $ret .= "<table>";
        $ret .= '<tr><td colspan="3">';
    ...
    public static function RegisterItemsMap($class)
    {
        ...
        if (isset(self::$ItemsMap[$tag]))
            throw new \Exception(Language::get("Tag name $tag is already defined in view manager!"));
