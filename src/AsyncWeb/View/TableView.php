...
class BasicDataViewCell implements DataViewCell {
    ...
    public function allowFilter() {
        return array(DV_OP_LIKE, DV_OP_NOT_LIKE, DV_OP_EQUAL, DV_OP_NOT_EQUAL, DV_OP_IS, DV_OP_IS_NOT,);
    }
    ...
    public function makeFilterOperant($trid, $key, $selected = null) {
        ...
        foreach ($this->allowFilter() as $filterType) {
            switch ($filterType) {
                case DV_OP_EQUAL:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
                    $ret.= '>=</option>';
                    break;
                ...
                case DV_OP_LIKE:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
                    $ret.= '>' . \AsyncWeb\System\Language::get("Contains") . '</option>';
                    break;
                case DV_OP_NOT_LIKE:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
                    $ret.= '>' . \AsyncWeb\System\Language::get("Deos not contain") . '</option>';
                    break;
                case DV_OP_IS:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
                    $ret.= '>' . \AsyncWeb\System\Language::get("Null value") . '</option>';
                    break;
                case DV_OP_IS_NOT:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
                    $ret.= '>' . \AsyncWeb\System\Language::get("Not null value") . '</option>';
                    break;
                case DV_OP_GT:
                    $ret.= '<option value="' . $filterType . '"';
                    if ($selected == $filterType) $ret.= ' selected="selected"';
    ...
    class DBValueDataViewCell extends BasicDataViewCell {
        ...
        public function allowFilter() {
            return array(DV_OP_EQUAL, DV_OP_NOT_EQUAL, DV_OP_LIKE, DV_OP_NOT_LIKE, DV_OP_IS, DV_OP_IS_NOT,);
        }
    }
    ...
    class DateDataViewCell extends BasicDataViewCell {
        ...
        public function allowFilter() {
            return array(DV_OP_EQUAL, DV_OP_GTEQ, DV_OP_LTEQ, DV_OP_IS, DV_OP_IS_NOT,);
        }
        protected function makeFilterValue($trid, $key, $default = null) {
            $DataTableId = $this->getTableId();
    ...
    class NumberDataViewCell extends BasicDataViewCell {
        ...
        public function allowFilter() {
            return array(DV_OP_EQUAL, DV_OP_GT, DV_OP_GTEQ, DV_OP_LT, DV_OP_LTEQ, DV_OP_NOT_EQUAL, DV_OP_IS, DV_OP_IS_NOT,);
        }
    }
    class CheckBoxDataViewCell extends BasicDataViewCell {
        ...
        public function allowFilter() {
            return array(DV_OP_EQUAL, DV_OP_NOT_EQUAL, DV_OP_IS, DV_OP_IS_NOT,);
        }
    }
    class FunctionItemDataViewCell extends BasicDataViewCell {
        ...
        public function show($id, $datarow, $type = "display") {
            ...
            if ($this->icon) {
                if (ViewConfig::$useFontAwesome) {
                    switch ($this->icon) {
                        case 'img/icons/blog_compose.png':
                            $ret.= '<i class="fa fa-pencil-square-o"></i>';
                        break;
                        case 'img/delete.png':
                            $ret.= '<i class="fa fa-times"></i>';
                        break;
                    }
                } else {
                    if (\AsyncWeb\IO\File::exists($this->icon)) {
                        $ret.= '<img src="/' . $this->icon . '" width="20" height="20" alt="' . $text . '" />';
                    } else {
                        $ret.= $text;
                    }
                    $ret.= '</a>';
    ...
    class UpdateItemDataViewCellV1 extends BasicDataViewCell {
        ...
        public function show($id, $datarow, $type = "display") {
            ...
            } elseif (\AsyncWeb\IO\File::exists("img/icons/blog_compose.png")) {
                $ret.= '<img src="/img/icons/blog_compose.png" width="20" height="20" alt="' . \AsyncWeb\System\Language::get("Edit item") . '" />';
            } else {
                $ret.= \AsyncWeb\System\Language::get("Edit item");
            }
            $ret.= '</a>';
            if ($this->inputFilter) $ret = $this->inputFilter->filter($ret, $datarow);
            return $ret;
    ...
    class UpdateItemDataViewCell extends FunctionItemDataViewCell {
        public function __construct() {
            parent::__construct(\AsyncWeb\System\Language::get("Edit item"), \AsyncWeb\System\Path::make(array("%getTableId%___ID" => "%id%", "%getTableId%___UPDATE1" => "1", "AJAX" => "")), array("%id%" => "id", "%getTableId%" => "%getTableId%"), "img/icons/blog_compose.png", false);
        }
    }
    class DeleteItemDataViewCellV1 extends BasicDataViewCell {
        ...
        public function show($id, $datarow, $type = "display") {
            ...
            } elseif (\AsyncWeb\IO\File::exists("img/delete.png")) {
                $ret.= '<img src="/img/delete.png" width="20" height="20" alt="' . \AsyncWeb\System\Language::get("delete") . '" />';
            } else {
                $ret.= \AsyncWeb\System\Language::get("delete ");
            }
            $ret.= '</a>';
            if ($this->inputFilter) $ret = $this->inputFilter->filter($ret, $datarow);
            return $ret;
    ...
    class DeleteItemDataViewCell extends FunctionItemDataViewCell {
        public function __construct() {
            parent::__construct(\AsyncWeb\System\Language::get("delete"), \AsyncWeb\System\Path::make(array("%getTableId%___ID" => "%id%", "%getTableId%___DELETE" => "1", "AJAX" => "")), array("%id%" => "id", "%getTableId%" => "%getTableId%"), "img/delete.png", \AsyncWeb\System\Language::get("confirm delete"));
        }
    }
    class TextDataViewCell extends BasicDataViewCell {
        ...
        public function allowFilter() {
            return array(DV_OP_LIKE, DV_OP_NOT_LIKE,);
        }
    }
    ...
    class BasicTHViewCell implements THViewCell {
        ...
        public function __construct() {
            ...
            if ($this->config->getDVC()) {
                $this->config->getDVC()->setTHViewCell($this);
            }
        }
                ...
                switch ($lastbinding) {
                    ...
                    if ($depth == 0 || $new) {
                        switch ($filter->getOperator()) {
                            case DV_OP_EQUAL:
                                if ($row[$filter->getCol() ] == $filter->getValue()) {
                                    $r = true;
                                } else {
                                    $r = false;
                                }
                            ...
                            break;
                            case DV_OP_LIKE:
                                if (stripos(\AsyncWeb\Text\Texts::clear($row[$filter->getCol() ]), \AsyncWeb\Text\Texts::clear($filter->getValue())) !== false) {
                                    $r = true;
                                } else {
                                    $r = false;
                                }
    ...
    class DataFilterItem {
        ...
        public function __construct($col, $operator, $value, $binding) {
            ...
            $this->col = $col;
            if ($operator != DV_OP_EQUAL && $operator != DV_OP_NOT_EQUAL && $operator != DV_OP_LTEQ && $operator != DV_OP_LT && $operator != DV_OP_GTEQ && $operator != DV_OP_GT && $operator != DV_OP_LIKE && $operator != DV_OP_NOT_LIKE && $operator != DV_OP_IS && $operator != DV_OP_IS_NOT && $operator != DV_OP_NULL) {
                throw new \Exception("Operator $operator has not been recognized");
            }
            if ($binding != DV_BINDING_AND && $binding != DV_BINDING_OR && $binding != DV_BINDING_AND_LB && $binding != DV_BINDING_RB_AND && $binding != DV_BINDING_OR_LB && $binding != DV_BINDING_RB_OR && $binding != DV_BINDING_NONE) {
    ...
    class TSAjax {
        ...
        public static function makeTableMenuScript($THViewCell, $showmenu = false) {
            ...
            if ($showmenu) {
                ...
                $i = 0;
                if ($tv) {
                    foreach ($tv->getTableMenuItems() as $tmi) {
                        $i++;
                        $ret.= $tmi->showMenu();
                    }
                }
    ...
    class TableMenuExportXML extends BasicTableMenuItem {
        ...
        private function make_export() {
            ...
            if (!$thr) exit;
            $ret = '<Data>';
            while ($datarow = $this->TableView->getDataSource()->next()) {
                $i++;
                $ret.= '<Item>';
...
<body><table id="data_table_' . $id . '" class="table data_table_' . $prefix . '">';
    ...
    class TableView implements View {
        ...
        public function show() {
            ...
            while ($datarow = $this->datasource->next()) {
                $c++;
